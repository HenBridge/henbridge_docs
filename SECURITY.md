...
HenBridge operates a formal bug bounty program for all repositories in the
HenBridge ecosystem. See the [program specification](docs/bug-bounty-program.md)
for:
...
Once `henbridge-web` and `henbridge-contract` exist and ship code, report vulnerabilities in those systems in their own repositories, following their respective `SECURITY.md` files.

## Supported versions

HenBridge is pre-alpha, Stellar **testnet** only. No production deployment
exists yet, so there is no supported-version matrix to publish.

## Bug bounty program

Once `henbridge-web` and `henbridge-contract` ship code, a formal
